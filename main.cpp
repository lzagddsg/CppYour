...
int main() {
	cout << "Thanks using CppYour! ^_^" << endl << "You can type HELP for helping. " << endl;
	string cmd;
	while (true) {
		cout << "Command>> ";
		cin >> cmd;
		if (cmd == "HELP") {
			cout << "Type in compile command to compile and type in running command to run your .exe file. You could type ABA to know more about the author and ABC for knowing more about this app. For exit, you can type EXIT to exit the LBW (little black window). If the GUI didn`t close, you should click X to close it. " << endl;
		} else if (cmd == "ABA") {
			cout << "Author: Lzagddsg (H2O Studio) in GitHub. " << endl << "H2O Studio: a studio founded by two students. " << endl;
		} else if (cmd == "ABC") {
			cout << "CppYour: a C++ IDE. " << endl;
		} else if (cmd == "EXIT") {
			cout << "Bye~ " << endl;
			return 0;
		} else {
			...
			for (int i = 0; i < cmd.length(); i++) {
				c[i] = cmd[i];
			} cout << "Running your command... " << endl;
			system(c);
		}
	}
}
